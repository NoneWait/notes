...
for t in range(1, T):
    for s in range(0, N):
        ...
        seq_score = [viterbi[s_, t-1]*transition_p[s_][s]*emission_p[s][ob[t]] for s_ in range(0, N)]
        
        viterbi[s, t] = max(seq_score)
        backpointer[s, t] = np.argmax(seq_score)
...
for t in range(1, T):
    bestpath.append(backpointer[bestpathpointer, t])
