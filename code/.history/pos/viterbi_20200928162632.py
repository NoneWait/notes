...
viterbi = np.zeros((N+1, T+1))
backpointer = np.zeros((N+1, T+1))

# 初始化
for s in range(1, N+1):
    viterbi[s, 1] = transition_p[0][s-1]
    backpointer[s, 1] = 0
...
for t in range(2, T+1):
    for s in range(1, N+1):
        seq_score = [viterbi[s_, t-1]*transition_p[s_][s]*emission_p[s][ob[t]] for s_ in range(1, N+1)]
